...
```bash
# Pull the image
$ docker pull kulkarnianup/jenkins:lts-k8s-20190827
```
...
$ docker run -d --name jenkins -p 8080:8080 -v /var/run/docker.sock:/var/run/docker.sock kulkarnianup/jenkins:lts-k8s-20190827
...
If you are using the pre-built image `kulkarnianup/jenkins:lts-k8s`, you can install the helm chart with
```bash
